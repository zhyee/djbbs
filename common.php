...
if (!$accessToken || !preg_match('/^[0-9a-w]+$/', $accessToken))
{
...
function TagsDiff($Arr1, $Arr2)
{
	...
	foreach ($Arr1 as $Key => $Item) {
	    $FItem = Filter($Item);
		if (mb_strlen($Item, "UTF-8") > $Config["MaxTagChars"] || isset($Arr2[strtolower(trim($Item))]) || strpos("|", $Item) || !preg_match('/^[a-zA-Z0-9\x80-\xff\-_\s]{1,' . $Config['MaxTagChars'] . '}$/i', $Item) || $Item != $FItem['Content']) {
			unset($Arr1[$Key]);
		} else {
...
{
	global $DB, $CurUserID, $MCache, $Config;
	if ($UserID == 0) {
		$UserID = $CurUserID;
	}
	if ($NewUserInfo) {
...
/*if ($CurUserExpirationTime > $TimeStamp && $CurUserExpirationTime < ($TimeStamp + 2678400) && $CurUserID && $CurUserCode) {
	...
	if (!$TempUserInfo) {
        ...
        $TempUserInfo['UserName'] = GetCookie('UserName');

		if ($MCache && $TempUserInfo) {
			$MCache->set(MemCachePrefix . 'UserInfo_' . $CurUserID, $TempUserInfo, 86400);
		}
	}
	if ($TempUserInfo && HashEquals(md5($CurUserExpirationTime . SALT), $CurUserCode)) {
