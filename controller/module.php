...
$ModuleID = $id = intval(Request('Get', 'id'));

$ModuleInfo = $DB->row('SELECT * FROM ' . PREFIX . 'module WHERE ModuleID  = ? AND IsDel = 0', array($id));
...
$BoardsArray = $DB->query('SELECT * 
	FROM ' . PREFIX . 'boards 
	WHERE IsEnabled=1 AND ModuleID = ? AND GroupID = ?
	ORDER BY TotalPosts DESC 
	LIMIT ' . ($Page - 1) * $Config['TopicsPerPage'] . ',' . $Config['TopicsPerPage'], array($id, $CurGroupID));
...
/*    if (date('Ymd', $TimeStamp) != date('Ymd', $Board['MostRecentPostTime']))
    ...
    $TotalPosts = $DB->single("SELECT COUNT(ID) FROM " . PREFIX . "topics WHERE GroupID = ? AND BoardID = ? AND IsDel = 0",
        array($CurGroupID, $BoardID));
    $TodayPosts = $DB->single("SELECT COUNT(ID) FROM " . PREFIX . "topics WHERE GroupID = ? AND BoardID = ? AND IsDel = 0 AND PostTime >= ?",
        array($CurGroupID, $BoardID, strtotime(date('Y-m-d'))));

    $BoardsArray[$k]['TotalPosts'] = $TotalPosts;
    $BoardsArray[$k]['TodayPosts'] = $TodayPosts;
...
$PageTitle = $ModuleInfo['ModuleName'];
